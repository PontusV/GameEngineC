...
/* Benefit of handles: These handles do not leave dangling pointers.
	...
	*******IDEA******** (TO BE REMOVED FROM DOCUMENTATION)
	Save handles by looping through array and storing only the active entries in file. Save entry index + their data.
	Reinitialization of pointers in Entry has to be made after its respective entity has been loaded.
	After all entities have been loaded there could be a loop checking that any loaded entry does not have a nullptr.
*/
