...
void VerticalLayoutGroup::updateLayout() {
	...
	if (rectTransform) {
		...
		for (std::size_t i = 0; i < elements.size(); i++) {
			...
			// Set position
			transform->setX(x - transform->getRectOffset().x - rectTransform->getRectOffset().x - element.size.x * anchor.x);
			transform->setY(y - transform->getRectOffset().y - rectTransform->getRectOffset().y);
